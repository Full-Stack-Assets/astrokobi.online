    ...
    <article className="mx-auto max-w-4xl px-5 py-14 sm:px-8 sm:py-24">
      ...
      <header className="mb-12 max-w-3xl">
        <div className="mb-4 flex items-center gap-3 text-xs uppercase tracking-[0.2em] text-muted">
          <Link href={`/categories/${frontmatter.category}`} className="border border-accent px-2 py-0.5 text-accent transition-colors hover:bg-accent hover:text-ink">
        ...
        <h1 className="font-display text-5xl font-medium leading-[.94] tracking-[-.05em] sm:text-7xl">
          <span className="gradient-text">{frontmatter.title}</span>
        </h1>
      ...
      {/* Hero */}
      <div className="glass-card mb-14 aspect-[16/8] overflow-hidden">
      ...
      {frontmatter.tags?.length > 0 && (
        <div className="mt-10 flex flex-wrap gap-2">
          {frontmatter.tags.map((t) => (
            <Link key={t} href={`/tags/${t}`} className="border border-white/20 px-2 py-1 text-[11px] uppercase tracking-widest text-muted transition-all hover:border-accent hover:text-accent hover:shadow-[0_0_14px_-6px_var(--color-accent)]">
              #{t}
            </Link>
          ))}
        </div>
